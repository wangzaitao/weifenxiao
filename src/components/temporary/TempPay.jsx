...
class TempPay extends React.Component {
  ...
  constructor(props, context) {
    ...
    var t = this.props.location.query.t;
    var tt = this.props.location.query.tt;
  ...
  onConfirmPayClick () {
    if ((this.state.totalPrice === 0) || (!this.state.useBalance && !this.state.useOtherPay)) {
      return false;
    } else {
      this.setState({isPaying: true});
      if (!this.state.useOtherPay || !this.state.otherPayAmount) {
        let apiData = {}, activityItem;
        ...
        if(this.state.activityId){//单商品支付
          ...
          ContentAPI.payByBalance(this.state.activityId, apiData, function (data) {
            if (Object.keys(data).length) {
              ...
              browserHistory.push({
                pathname: '/temporary/pay_result/' + this.state.activityId + '/balance/' + this.state.amount,
                query: {count: data.order_numbers.length},
                state: {
                  activityList: [{
              ...
              console.log('Pay by balance failed!');
          ...
          ContentAPI.payBatchByBalance(apiData, function (data) {
            let successList = data.success_list, successItem,
              failList = data.fail_list,
              originActItem, targetActItem,
              targetActivityList = [],
              _totalAmount = 0;
            if(successList.length === 0){
              ...
              browserHistory.push({
                pathname: '/temporary/pay_result/balance/' + _totalAmount,
        ...
        ContentAPI.getPayID({type: this.state.otherPayType}, function (payId) {
          if (payId) {
            ...
            if(this.state.activityId){
              returnUrl = window.location.origin + '/temporary/pay_result/' + this.state.activityId + '/pay/' + payId;
              apiData.activity_id = this.state.activityId;
            }else{
              returnUrl = window.location.origin + '/temporary/pay_result/pay/' + payId;
              apiData.buy_list = JSON.stringify(this.activityIdList);
            }
