  ...
  render() {
    ...
    if(orderNumbers.length > 7) {
      ...
      if(props.payStatus === 2) {
        viewNumberLink = <CustomLink key={7} to={'/numbers/' + props.id} className="num-count-item fr txt-one-line" isOuterURL={true}>查看全部</CustomLink>;
      }
    }
...
class TempPayResult extends React.Component {
  ...
  componentWillMount () {
    let targetActivityList = this.props.location.state.activityList;
    Auth.updateUserInfo();
      ...
      ContentAPI.getPayStatus(this.state.payId, function (data) {
        if (Object.keys(data).length) {
          this.setState({
            ...
            count: data.extend.refer_order_numbers.length
          });
        }
      }.bind(this));
    }
...
TempPayResult.contextTypes = {
  history: PropTypes.object.isRequired,
  location: React.PropTypes.object
};

export default TempPayResult;
