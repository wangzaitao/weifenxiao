...
export function postReceipts(data) {
  ...
  var ajaxData = {
    type: 'POST',
    url: GlobalConfig.API.HOST + GlobalConfig.API.POST_Receipt
  };
  ajaxData.data = data;
  cpromise = promiseAjax(ajaxData);
  return cpromise
...
export function deleteReceipts(id) {
  var deleteReceiptsURL = (GlobalConfig.API.HOST + GlobalConfig.API.DELETE_RECEIPT);
  var cpromise;
  var ajaxData = {
    type: 'POST',
    url: deleteReceiptsURL + "?id=" + id + "&rd=" + Math.random()
  };
  cpromise = promiseAjax(ajaxData);
  return cpromise
    .then((res) => {
